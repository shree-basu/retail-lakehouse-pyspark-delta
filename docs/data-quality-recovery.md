...
Quarantine stores entity, serialized key, record hash, batch ID, business date, source file, all reason codes, rejected payload, and timestamp. Its `(business_date, source_batch_id, entity, record_hash)` merge key makes exact retries safe without suppressing equivalent rejected records from different business dates.
...
2. For a transient executor/filesystem failure, rerun the same immutable path. Bronze transaction identity uses the complete delivery identity and downstream merges make the replay safe.
